...
void *front(queue q)
{
	...
	// produce an error if called on an empty queue
	if (is_empty_queue(q))
	{
		trace("front: empty queue; quitting");
...
void rear(queue q)
{
	...
	if (is_empty_queue(q))
	{
		trace("rear: empty queue; quitting");
...
{
	//COMPLETE ME!
	node n; // node to be added to the queue
	node c; // current node in the traversal
	...
	if (is_empty_queue(q))
	{
		// if the queue is empty, set the new node to begin the queue
		q->first = n;
	}
	else
	{
		// begin traversing at the beginning of the queue
		c = q->first;

		// continue traversing the queue until the end is reached
