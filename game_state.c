...
void init_game_state(game_state *gp)
{
	...
	trace("init_game_state: init_game_state starts");	//for debug purposes only
	...
	*gp = (game_state)malloc(sizeof(struct game_state_int));
	...
	for (x = START_OF_ARRAY; x < DIMENSION; x++)
	{
		...
		{
			 (*gp)->board[x][y] = false;
			//init_square_state(gp, x, y);
		}
	}

	trace("init_game_state: init_game_state ends");	//for debug purposes only.
...
{
	trace("get_square: get_square starts ");

		return(g->board[r - 1][c - 1]);
	
	trace("get_square: get_square  finishes");
}
...
{
	//COMPLETE ME!
	trace("valid: valid starts ");

	// compare r, c to board dimentions(taking into consideration that r, c could be 0 or below)
	if ((0 < r) && (0 < c) && (r <= DIMENSION) && (c <= DIMENSION))
	{
		//c,r is on the board
		trace("valid: valid true finishes");
		return true;
	...
	{
		//off the board
		trace("valid: valid false  finishes");
		return false;
		

	}

	trace("valid: valid  finishes");
}
...
{
	//COMPLETE ME!
	int y = 1 ;	// column starts of given game_state

	trace("row_clear: row_clear starts");

	while (y <= DIMENSION)
	{
		if (taken(g, r, y))
		{
			trace("row_clear: row_clear finishes with false");
	...
	trace("row_clear: row_clear finishes with true");
...
{
	//COMPLETE ME!
	int x = 1;	// row starts of given game_state

	trace("column_clear: column_clear starts");

	while (x <= DIMENSION)
	{
		if (taken(g, x, c))
		{
			trace("column_clear: column_clear finishes with false");
	...
	trace("column_clear: column_clear finishes with true");
...
bool taken(game_state g, int r, int c)
{
	...
	//const int DIF = 1;
	return (get_square(g, r, c) != false);

	trace("taken: taken finishes");
...
{
	//COMPLETE ME! 
	trace("land: land starts and finishes");
...
game_state clone(game_state g)
{
	...
	for (int r = 0; r < DIMENSION; r++)
	{
		...
		{
			//init_game_state(&n);
			// determine if the space has visisted on previously
			//if (occupied(g->board[r][c]) != true)
			if(taken(g,r,c))
			{
				//init_square_state(&(g->board[r][c]), r, c); // create new square

				//occupy(g->board[r][c]); // undate occupy status
				//land(g, r,c); //undate occupy status

				n->board[r][c] = (g->board[r][c]);
			}
		}
	}
